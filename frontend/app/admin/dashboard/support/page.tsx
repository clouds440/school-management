...
export default function SupportPage() {
    ...
    const columns: Column<SupportTicket>[] = [
            ...
            accessor: (row) => (
                <div className="flex items-start gap-4 min-w-0">
                    ...
                    <div className="min-w-0 flex-1">
                        <h4 className="text-sm font-black text-gray-900 leading-tight break-words flex items-center gap-2">
                            {row.organization?.name}
                            <span className={`text-[9px] uppercase font-bold px-1.5 py-0.5 rounded-md shrink-0 ${row.organization?.status === 'REJECTED' ? 'bg-red-100 text-red-600' :
        ...
        },
        {
            header: 'Message',
            accessor: (row) => (
                <div className="text-xs text-gray-700 bg-gray-50/50 p-3 rounded-xl border border-gray-100 italic break-words max-w-sm">
                    "{row.message}"
                </div>
            )
        },
                ...
                <div className="flex flex-col gap-2 shrink-0 sm:items-end w-40">
                    {row.topic === 'ACCOUNT_STATUS' && (row.organization?.status === 'REJECTED' || row.organization?.status === 'SUSPENDED') && (
                        <button
                            ...
                            className="w-full bg-emerald-50 text-emerald-700 hover:bg-emerald-600 hover:text-white px-3 py-2 rounded-lg font-bold text-xs flex items-center justify-center gap-1.5 transition-all active:scale-95 border border-emerald-100"
                        >
                    ...
                    <button
                        ...
                        className="w-full mt-auto bg-indigo-50 text-indigo-700 hover:bg-indigo-600 hover:text-white px-3 py-2 rounded-lg font-bold text-xs flex items-center justify-center gap-1.5 transition-all active:scale-95 border border-indigo-100"
                    >
    ...
    return (
        <div className="flex flex-1 flex-col p-6 sm:p-10 w-full animate-fade-in-up">
            <h1 className="text-3xl font-black text-gray-900 tracking-tight mb-8">Support Mails</h1>

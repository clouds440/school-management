...
export default function OrgLayout({ children }: { children: React.ReactNode }) {
    ...
    const links = useMemo((): SidebarLink[] => {
        ...
        if (user?.role === Role.ORG_ADMIN || user?.role === Role.ORG_MANAGER) {
            ...
            orgLinks.push({ id: 'SECTIONS', label: 'Sections', href: `/${orgSlug}/sections`, icon: BookOpen, badge: stats?.SECTIONS });
            ...
            orgLinks.push({ id: 'SECTIONS', label: 'My Sections', href: `/${orgSlug}/sections`, icon: BookOpen, badge: stats?.SECTIONS });
    ...
    return (
        <DashboardLayout
            title={orgData?.name || 'Dashboard'}
            links={links}
            bottomLinks={bottomLinks}
            brandHref={brandHref}
        >
            {showOverlay ? <StatusOverlay /> : children}
        </DashboardLayout>
    );
}

...
export default function AddStudentPage() {
    ...
    useEffect(() => {
        if (user && user.role !== Role.ORG_ADMIN && user.role !== Role.ORG_MANAGER && user.role !== Role.TEACHER) {
            router.push(`/${orgSlug}/students/${user.userName}`);
        }
    }, [user, orgSlug, router]);
    ...
    return (
        <div className="flex flex-col">
            ...
            <div className="bg-card/80 backdrop-blur-xl rounded-sm shadow-2xl border border-border p-6 md:p-12 text-card-text">

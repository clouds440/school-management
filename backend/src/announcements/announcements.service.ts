...
export class AnnouncementsService {
    ...
    async createAnnouncement(dto: CreateAnnouncementDto, user: CurrentUser) {
        ...
        if (dto.targetType === TargetType.GLOBAL) {
            if (user.role !== Role.SUPER_ADMIN && user.role !== Role.PLATFORM_ADMIN) {
                throw new ForbiddenException('Only System Admins can create GLOBAL announcements.');
            }
        } else if (dto.targetType === TargetType.ORG) {
            if (user.role === Role.TEACHER || user.role === Role.ORG_MANAGER) {
                throw new ForbiddenException(`As an ${user.role.replace('_', ' ')}, you are not authorized to create organization-wide announcements.`);
            }
            if (dto.targetId && dto.targetId !== user.organizationId && user.role !== Role.SUPER_ADMIN && user.role !== Role.PLATFORM_ADMIN) {
                 throw new ForbiddenException('Cannot target an Organization you do not belong to.');
            }
        } else if (dto.targetType === TargetType.ROLE) {
            if (user.role === Role.TEACHER) {
                throw new ForbiddenException('Teachers are not authorized to create role-based announcements.');
            }
            
            const targetRole = dto.targetId as Role;
            const isPlatformTarget = targetRole === Role.SUPER_ADMIN || targetRole === Role.PLATFORM_ADMIN;

            if (user.role !== Role.SUPER_ADMIN && user.role !== Role.PLATFORM_ADMIN && isPlatformTarget) {
                throw new ForbiddenException(`As an ${user.role.replace('_', ' ')}, you cannot target Platform or Super Administrators.`);
            }

            if (user.role === Role.ORG_MANAGER && targetRole === Role.ORG_ADMIN) {
                throw new ForbiddenException('Org Managers are not authorized to target Org Administrators.');
            }
            ...
            if (user.role === Role.TEACHER) {
                ...
                if (!dto.targetId || !sectionIds.includes(dto.targetId)) {
                    throw new ForbiddenException('You can only send announcements to your assigned sections.');
                }
            }
        }
        ...
        const announcement = await this.prisma.announcement.create({
            data: {
                ...
                // @ts-ignore: Prisma schema update pending restart
    ...
    async getAnnouncements(user: CurrentUser, page: number = 1, limit: number = 10) {
        ...
        const conditions: Prisma.AnnouncementWhereInput[] = [
            ...
            { targetType: TargetType.ROLE, targetId: user.role, organizationId: user.organizationId },
            // If they are platform admin, they might see more, but keep it simple for now
        ];

        // If User is Teacher/Student, fetch their sections
        if (user.role === Role.TEACHER) {
            const profile = await this.prisma.teacher.findUnique({ where: { userId: user.id }, include: { sections: true } });
            if (profile && profile.sections.length > 0) {
                conditions.push({ targetType: TargetType.SECTION, targetId: { in: profile.sections.map(s => s.id) } });
            }
        } else if (user.role === Role.STUDENT) {
            const profile = await this.prisma.student.findUnique({ where: { userId: user.id }, include: { enrollments: true } });
            if (profile && profile.enrollments.length > 0) {
                conditions.push({ targetType: TargetType.SECTION, targetId: { in: profile.enrollments.map(e => e.sectionId) } });
            }
        }
